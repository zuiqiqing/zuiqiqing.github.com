...
if ( $user->ID ) {
	...
	if ( get_option('comment_registration') || 'private' == $status )
		err(__('对不起，你必须登录后才能发表评论.')); // 將 wp_die 改為錯誤提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('错误: 请填写昵称和邮箱！ (name, email).') ); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
		err( __('错误：请输入有效的电子邮件地址.') ); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
	err( __('错误：请输入评论.') ); // 將 wp_die 改為錯誤提示
...
?>
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID ?>">
		<div id="comment-<?php comment_ID(); ?>" class="comment_body">
			<div class="author"><?php echo get_avatar($comment,$size='30'); ?></div>
